...
class GeoSim():
    ...
    The GeoSim model of individual human mobility consists of the following mechanisms [THSG2015]_: 
    ...
    **Individual Exploration**. If the agent :math:`a` is currently in location :math:`i`, and explores a new location without the influence of its social contacts, then the new location :math:`j \\neq i` is an unvisited location for the agent (:math:`i \\in exp_{a}`) 
    and it is selected with probability proportional to :math:`\\frac{1}{|exp_{a}|}`. The number of distinct locations visited, :math:`S`, is increased by 1.
    ...
    **Social Return**. If the agent :math:`a` is currently in location :math:`i`, and returns to a previously visited location with the influence of a social contact, it first selects a social contact :math:`c` 
    ...
    ----------
    name : str, optional
        the name of the instantiation of the GeoSim model. The default value is "GeoSim".
    rho : float, optional
    ...
    min_wait_time_minutes : int
        minimum waiting time between two movements, in hours. 
    ...
    def generate(self, start_date, end_date, spatial_tessellation, social_graph='random', 
                 n_agents=500, dt_update_mobSim = 24*7, indipendency_window = 0.5, random_state=None, log_file=None, verbose=0, show_progress=False): 
        ...
        #init. a progress bar with hourly resolution
        if show_progress:
        ...
        while self.current_date < self.end_date:
            ...
            for agent in range(self.n_agents):
                ...
                if location_id is None:                 
                    ...
                    #generate a random number for the spatial choice: Explore or Return respectively with probability pS^-gamma and 1-pS^-gamma                
